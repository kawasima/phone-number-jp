...
日本の電話番号体系は見た目以上に複雑です。固定電話の市外局番は2桁〜5桁で可変長であり、携帯電話・IP電話・フリーダイヤル・M2M番号などそれぞれ異なる桁数とルールを持ちます。さらに総務省による番号制度の改正（060番号の携帯電話への追加、020番号の14桁化など）が継続的に行われており、正確なパースとバリデーションを自前で実装・維持するのは困難です。

phone-number-jp は以下の課題を解決します:

- **可変長の市外局番を正しく分割** - 総務省の番号指定データに基づき、`0725999999` を `0725-99-9999`（和泉MA）として正確にパースします
- **番号種別の自動判定** - 入力された番号が固定電話・携帯電話・IP電話のいずれかを自動で判定し、対応する型のオブジェクトを返します
- **総務省の制度改正に追従** - 060携帯番号、M2M 14桁番号、付加的役務電話番号（0120/0800等）など、最新の番号体系に対応しています
- **総務省データからの自動生成** - 固定電話番号の市外局番・市内局番データは総務省のExcelファイルから自動生成でき、番号指定状況の変更に追従できます
